...
void blit_sprite_aa_edges(ImageRecord *sprite, int dst_x, int dst_y, uchar far *palette) {
    uchar far *src;
    int y;
    int cur;
    int color;
    ...
    *(uchar far **)&g_pAaPaletteBuf = palette;
    ...
    while (sprite->nHeight > y) {
        ...
        while (x < sprite->nWidth) {
            cur = (char)*((uchar far *)MK_FP(FP_SEG(src), x * sprite->nHeight + y) + FP_OFF(src));
            if (prev == 0 && cur != 0) {
