...
extern int combatenc_skill_check_random(CombatActor *attacker, CombatActor *target, int base_skill,
                                        int quarrel_kind);
extern void combatenc_ai_pick_target_by_role(CombatActor *actor, int max_distance, uint role_filter,
...
extern void combatenc_persist_actors_to_temp(int srcEncounterId);
...
void far combatenc_release_actors(void) {
    ...
    for (i = g_combat_count_B - 1; i >= 0; i--) {
        g_combat_actors_B[i].actor_record->dirty_flag = 1;
        g_combat_actors_B[i].actor_record->persist_gate = 1;
