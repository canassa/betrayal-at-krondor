...
int music_chunk_load_and_link(BakFile *file, char mode) {
    ...
    *(unsigned short far *)&node->bPriority = (unsigned char)scratch;
    ...
    codec_kind = (node->wFlags & 1) ? 4 : 7;

    size = size - 4;
    node->pData = 0;

    if (mode == 'c') {
        if ((node->pData = pool_acquire_buffer(size, codec_kind)) == 0 ||
        ...
        if ((node->pData = audres_load_chunk_by_mode(file, (unsigned)size,
                                                     ...
                                                     (unsigned *)out_size, codec_kind)) == 0) {
            release_buffer(node, 3);
            return 0;
        }
    } else {
        if ((node->pData =
                 stream_load_to_buffer(file, size, (unsigned long *)out_size, codec_kind)) == 0) {
            release_buffer(node, 3);
            return 0;
        }
    }
